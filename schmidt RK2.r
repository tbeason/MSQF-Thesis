...
SchmidtRK2 <- function(alpha=1,beta=1,gamma=1,delta=1,d=0.1,rho=0.05,n0=0.4,p0=1.05,dt=0.01,N=25,pf=1,Tconf=1,Fconf=1){
  T <- N/dt+1
  n_plus <- numeric(T)
  price <- numeric(T)
  util <- numeric(T)
  ...
  U <- function(n,p)
  {
    alpha*Tconf*gamma*delta*(2*n-1) +beta*Fconf*(pf-p)
  }
